...
exports.fetchAllTodos = asyncHandler(async (req, res) => {
  ...
  let result = allTodos.map((todo) => ({
    ...
    createdAt: format(todo.dueDate, "dd/MM/yyyy"),
    updatedAt: format(todo.dueDate, "dd/MM/yyyy"),
  }));

  if (allTodos.length === 0) throw new ErrorHandler(404, "No todos found");

  res.status(200).json({ success: true, message: "All todos", result });
});
...
// case 1 ==> multiple conditions on same fields  --> the last condition will be considered
// case 2 ==> multiple conditions on different fields --> implicit and
